...
class LSTMModel(nn.Module):
    def __init__(self, input_size=1, hidden_layer_size=150, output_size=1):
        ...
        self.lstm = nn.LSTM(input_size, hidden_layer_size, num_layers=4, batch_first=True)  # 增加LSTM层数
        self.dropout = nn.Dropout(0.3)  # 增加dropout
        self.linear = nn.Linear(hidden_layer_size, output_size)

    def forward(self, x):
        h_0 = torch.zeros(4, x.size(0), self.hidden_layer_size)  # 4层LSTM的隐藏状态
        c_0 = torch.zeros(4, x.size(0), self.hidden_layer_size)
