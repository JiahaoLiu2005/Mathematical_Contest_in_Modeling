...
class BiLSTMModel(nn.Module):
    def __init__(self, input_size=1, hidden_layer_size=150, output_size=1):
        ...
        self.bilstm = nn.LSTM(input_size, hidden_layer_size, num_layers=4, batch_first=True, bidirectional=True)  # 使用双向LSTM
        self.dropout = nn.Dropout(0.3)
        self.linear = nn.Linear(hidden_layer_size * 2, output_size)  # 由于是双向的，输出维度要乘以2

    def forward(self, x):
        h_0 = torch.zeros(8, x.size(0), self.hidden_layer_size)  # 4层 * 2方向 = 8
        c_0 = torch.zeros(8, x.size(0), self.hidden_layer_size)
...
# 使用函数预测给定的时间步
time_steps_to_predict = [100, 101, 102]  # 替换为你要预测的时间步
predicted_values = predict_for_time_steps(time_steps_to_predict)

# 输出预测结果
for i, val in enumerate(time_steps_to_predict):
    print(f"时间步 {val}: 预测值 = {predicted_values[i][0]:.4f}")
